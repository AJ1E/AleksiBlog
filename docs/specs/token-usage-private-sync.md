...
5. `aleksiz-ai-usage.service` runs in snapshot-only mode on `127.0.0.1:8787`; the browser sees data only through Astro's authenticated BFF. Every code release restarts an active helper, or starts an enabled-but-stopped helper, so it loads the scripts from the new `current` release.
