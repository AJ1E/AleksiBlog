...
- 影响：为了显示国家、VPN/代理/机房判断，访客公网 IP 会由服务器转发到 `ip.net.coffee`。这属于个人信息处理，应在公开上线前决定是否保留，并写入简短隐私说明。
- 处理：推荐默认保留“本次访问的大致地区”而不展示完整 IP；若保留风险判断，应在页脚或隐私页说明第三方服务和用途。若不愿向第三方传输访客 IP，则关闭该卡片或改为本地数据库方案。
