...
- 影响：为了显示国家/地区、ASN 与 VPN/代理/机房判断，访客公网 IP 会由服务器转发到 IPinfo、Proxycheck 和 Net.Coffee。这属于个人信息处理。
- 处理：地区定位与风险识别必须保持分层：Net.Coffee/IPinfo 用于地区回退，Proxycheck/Net.Coffee 仅用于风险信号，不能用风险服务的城市字段覆盖地区结果。默认仅展示脱敏 IP 和大致地区；隐私页需说明服务与用途。
